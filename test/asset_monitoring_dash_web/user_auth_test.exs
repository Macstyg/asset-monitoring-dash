defmodule AssetMonitoringDashWeb.UserAuthTest do
  use AssetMonitoringDashWeb.ConnCase, async: true

  alias AssetMonitoringDash.Accounts
  alias AssetMonitoringDash.Accounts.Scope
  alias AssetMonitoringDashWeb.UserAuth
  alias Phoenix.LiveView

  import AssetMonitoringDash.AccountsFixtures

  @remember_me_cookie "_asset_monitoring_dash_web_user_remember_me"
  @remember_me_cookie_max_age 60 * 60 * 24 * 14

  setup %{conn: conn} do
    conn =
      conn
      |> Map.replace!(:secret_key_base, AssetMonitoringDashWeb.Endpoint.config(:secret_key_base))
      |> init_test_session(%{})
  ...
  describe "log_in_user/3" do
    ...
    test "writes a cookie if remember_me was set in previous session", %{conn: conn, user: user} do
      ...
      conn =
        ...
        |> Map.replace!(
          :secret_key_base,
          AssetMonitoringDashWeb.Endpoint.config(:secret_key_base)
        )
        |> fetch_cookies()
  ...
  describe "logout_user/1" do
    ...
    test "broadcasts to the given live_socket_id", %{conn: conn} do
      ...
      AssetMonitoringDashWeb.Endpoint.subscribe(live_socket_id)
  ...
  describe "disconnect_sessions/1" do
    test "broadcasts disconnect messages for each token" do
      ...
      for %{token: token} <- tokens do
        AssetMonitoringDashWeb.Endpoint.subscribe("users_sessions:#{Base.url_encode64(token)}")
      end
